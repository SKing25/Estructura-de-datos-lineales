class Complejo:
    ...
    def __truediv__(self, other: 'Complejo') -> 'Complejo':
        ...
        """
        denominador = (other._real ** 2) + (other._imag ** 2)

        if denominador == 0:
            raise ZeroDivisionError("No se puede dividir por un complejo cero")

        nueva_real = (self._real * other._real + self._imag * other._imag) / denominador
        nueva_imag = (self._imag * other._real - self._real * other._imag) / denominador
...
div = c1 / c2  # c1.__truediv__(c2)

print(suma)     # 3 + 7i
print(resta)    # 1 - 1i
print(mult)     # (2*1 - 3*4) + (2*4, 3*1)i = -10 + 11i
print(div)      # (a + bi)/(c + di) = [(ac + bd)/(c² + d²)] + [(bc - ad)/(c² + d²)]i
